...
function safeWebUrl(rawUrl: string): URL {
...
async function fetchHtml(urlValue: string, signal?: AbortSignal, redirects = 0): Promise<string> {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        'User-Agent':
          'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 PrismHarness/1.0',
...
function pageText(html: string, maximumCharacters: number): { title: string; content: string } {
...
function duckDuckGoResultUrls(html: string): string[] {
  const urls: string[] = []
...
async function searchAndReadWeb(
  query: string,
  settings: EffectiveHarnessSettings,
  signal?: AbortSignal
): Promise<{
  ...
  const searchHtml = await fetchHtml(searchUrl, signal)
  const candidates = duckDuckGoResultUrls(searchHtml)
  const pages: Array<HarnessSource & { content: string }> = []
  const perPageLimit = Math.max(
    2_000,
    Math.floor(settings.maxContextCharacters / settings.webPageCount)
  )
  for (const url of candidates.slice(0, 12)) {
    if (pages.length >= settings.webPageCount) break
    try {
      ...
      // Continue until the configured number of readable sources is reached.
    }
  }
  if (pages.length === 0) throw new Error('DuckDuckGo returned no readable source pages.')
  return {
    query,
