...
- **System Prompt Composition:** Injects OS version, CWD, username, local date/time (with dates explicitly identified as `MM/DD/YYYY`), active workflow system instructions, and tool constraints. Non-Harness Chat and Quick Launcher user messages also carry internal sent-at metadata for temporal context; the UI keeps this metadata hidden.
