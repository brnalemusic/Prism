...
- Signing follows local `commit.gpgsign`; Sign-off is opt-in. Co-authorship adds a `Co-authored-by` trailer only to commits created by Git Control. Existing commits are never rewritten and force push is not exposed.
